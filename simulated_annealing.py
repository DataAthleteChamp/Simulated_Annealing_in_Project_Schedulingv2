...
#
#             # Calculate deadlines if not present (set to 1.5x processing time)
#             for task in self.tasks:
#                 if 'deadline' not in task:
#                     task['deadline'] = task['processing_time'] * 1.5
#
...
#         """Calculate cost with comprehensive violation tracking"""
#         try:
...
#
#                 # Check precedence constraints (successors)
#                 for succ in task.get('successors', []):
#                     if succ in task_end_times:
...
#             precedence_penalty = precedence_violations * 1000
#             resource_penalty = resource_violations * 1000
#
#             total_cost = base_cost + precedence_penalty + resource_penalty
#
#             violations = {
...
#         """Tune SA parameters based on problem characteristics"""
#         try:
...
#
#             # Base parameters on problem size
#             if self.num_tasks < 50:  # Small problems
#                 base_temp = 1000
#                 base_alpha = 0.98
#                 base_iterations = 2000
#             elif self.num_tasks < 100:  # Medium problems
#                 base_temp = 2000
#                 base_alpha = 0.95
...
#             complexity_factor = (1 + resource_complexity) * (1 + successors_density)
#
#             initial_temperature = base_temp * complexity_factor
#             cooling_rate = base_alpha ** (1 / complexity_factor)
#             max_iterations = int(base_iterations * np.sqrt(complexity_factor))
#             min_temperature = initial_temperature * 0.01
#
...
#             print(f"Min Temperature: {min_temperature:.2f}")
#
#             return {
...
#         """Run optimization with comprehensive result tracking"""
...
#                 # Track progress
#                 if iteration % 100 == 0:
#                     acceptance_rate = moves_accepted / (iteration + 1)
#                     print(f"Iteration {iteration}, Cost: {current_cost:.2f}, "
#                           f"Temp: {temperature:.2f}, Accept Rate: {acceptance_rate:.2%}")
#
...
#         plt.subplot(2, 1, 1)
#         plt.plot(self.cost_history, 'b-', label='Cost')
...
#                     for resource, amount in self.tasks[task['task_id']]['resource_requirements'].items():
...
#         plt.title('Resource Violations Over Time')
#         plt.xlabel('Time')
...
#             task = self.tasks[i]
#             weight = (
#                 len(task.get('successors', [])),  # More successors first
#                 task['deadline'],  # Earlier deadlines first
#                 -task['processing_time']  # Shorter tasks first
#             )
...
#         """Generate neighbor with multiple move types"""
#         try:
...
#                 if j - i > 1:  # Only reverse if we have at least 2 elements
...
#                 insert_point = random.randint(0, len(neighbor))
...
#                 for succ in task.get('successors', []):
#                     if succ in task_end_times:
#                         start_time = max(start_time, task_end_times[succ])
#
#                 end_time = start_time + task['processing_time']
#                 task_end_times[task_id] = end_time
#
